...
class PengajuanSimperController extends BaseController
{
    protected SimperModel $simperModel;
    protected UnitRecomendationModel $unitRecomendationModel;
    ...
    public function index(): string
    {
        ...
        $data = [
            ...
            'simpers_waiting' => $this->simperModel->where("approval_status", "waiting")->orderBy("created_at", "DESC")->findAll(),
            'simpers_accept' => $this->simperModel->where("approval_status", "approved")->findAll()
        ];
        // $data['company'] = $companyModel->find($companyId);
        // $data['anggota'] = $anggotaModel->getAnggotaData($companyId);
        return view('dept_head/pengajuan_simper', $data);
    }

    
    public function detail($id)
    {
        $simper = $this->simperModel->find($id);
        if($simper){
            ...
            return view('dept_head/detail_simper.php', $data);
        }else{
    ...
    public function detailIDCard($id)
    {
        ...
        if($simper){
            ...
            return view('dept_head/detailIDCardSimper.php', $data);
        }else{
            echo "id card tidak ditemukan";
        }
    }

    public function postContinueDone()
    {
        $id = $this->request->getPost("id");
        
        $data = (object) [
            'approval_status' => "approved", // Menyimpan path file
        ];

        $this->simperModel->update($id,  $data);

        return redirect()->to("/dept_head/simper/detail/simper/".$id)->with("success", "Success! Simper telah selesai dibuat.");
    ...
    public function detailPengajuan($id)
    {
        ...
        if($simper){
            ...
            return view('dept_head/detail-pengajuan-simper.php', $data);
        }else{
            echo "id card tidak ditemukan";
        }
    }
    public function postInstanCreate()
    {
        ...
        return redirect()->to("/dept_head/simper")->with("success", "Success! Simper berhasil ditambah.");
        // return redirect()->to("/" . $outputPathMinePermit)->with("success", "Success! Simper berhasil ditambah.");
    }

    public function postAjukanSimper()
    {
        // dd($this->request->getPost());
        ...
        $qr_code = uniqid();
        $outputPathMinePermit = $this->generateIdCard(strtoupper($employeeName), strtoupper($nrp), strtoupper($jabatan), strtoupper($company), strtoupper($gol_darah), $masa_berlakuformat, $imageName, $qr_code);
        ...
        $data = (object) [
            ...
            'gol_darah' => $gol_darah,
            'masa_berlaku' => $inputMasaBerlaku,
            'pas_foto_image' => $imageName, // Menyimpan path file
            'simper' => $outputPathMinePermit, // Menyimpan path file
            // 'simper_belakang' => "/". $outputPathMinePermitBelakang, // Menyimpan path file
            'qr_code' => $qr_code, // Menyimpan path file
    ...
    //     return redirect()->to("/" . $outputPathMinePermitBelakang)->with("success", "Success! Simper berhasil ditambah.");
    // }

    public function updateTtdBgSimper($idSimper){

        $unitRecomendationModel = new UnitRecomendationModel();
        $unitRecomendations = $unitRecomendationModel->where("simper_id", $idSimper)->findAll();
        $newPath = $this->generateIdCardBelakang($unitRecomendations);    
        $data = [
            'simper_belakang' => "/" . $newPath
        ];
        $this->simperModel->update($idSimper, $data);
        return redirect()->to("/dept_head/simper/detail/simper/$idSimper")->with("success", "Success! Tanda tangan bagian belakang Simper berhasil diubah.");
    }

    private function generateIdCardBelakang($unitRecomendations)
    {
        $backgroundPath = 'simper/belakang_bg_compress.png';
        ...
        // NRP
        $y=0;
        foreach($unitRecomendations as $index => $unitRecomendation){    
            $type_kendaraan = $unitRecomendation['jenis_alat'] . " " . strtoupper($unitRecomendation['type_unit']);
            $xPosition3 = 840;
            if($index==0){
                $y = 1279;
            }else{
            ...
            foreach (str_split($type_kendaraan) as $char3) {
                imagettftext($bgImage, $fontSize3, 0, $xPosition3, $y, $textColor, $fontPath, $char3);
                $xPosition3 += imagettfbbox($fontSize3, 0, $fontPath, $char3)[2] + $spacing3;
            }
            $no = $index+1 . ".";
            $xPosition3 = 640;
            foreach (str_split($no) as $char3) {
                imagettftext($bgImage, $fontSize3, 0, $xPosition3, $y, $textColor, $fontPath, $char3);
                $xPosition3 += imagettfbbox($fontSize3, 0, $fontPath, $char3)[2] + $spacing4;
            }
            $type = $unitRecomendation['jenis_simper'];
            $xPosition3 = 2390;
            foreach (str_split($type) as $char3) {
                imagettftext($bgImage, $fontSize3, 0, $xPosition3, $y, $textColor, $fontPath, $char3);
                $xPosition3 += imagettfbbox($fontSize3, 0, $fontPath, $char3)[2] + $spacing4;
            }
        }
        ...
        $outputPath = 'uploads/simper/belakang_' . uniqid() . '.png';
    ...
    public function generateIdCardBelakangBg()
{
    $backgroundPath = 'id_card/id_card_belakangkosong_bg.jpg';
    ...
    // Mendapatkan informasi tentang gambar tanda tangan
    $addedImage = imagecreatefrompng("image/ttd-pak-aklis.png");

    // Pastikan transparansi tetap dipertahankan
    imagesavealpha($addedImage, true);
    imagealphablending($addedImage, false); // Matikan alpha blending untuk menjaga transparansi
    ...
    // Pastikan gambar resizedImage mendukung transparansi
    $transparent = imagecolorallocatealpha($resizedImage, 0, 0, 0, 127);
    imagefill($resizedImage, 0, 0, $transparent);
    imagealphablending($resizedImage, true); // Nyalakan alpha blending
    ...
    $positionX = 1438; // Margin dari kiri
    $positionY = 4000; // Margin dari atas
    ...
    $spacing3 = 10;
    $spacing4 = 6;
    $autorized = "Ahmad Akhlis";

    // autorized
    $xPosition3 = 1325;
    foreach (str_split($autorized) as $char3) {
    ...
    // Tentukan path untuk menyimpan gambar
    $outputPath = 'uploads/simper/belakang_bg'. '.png';
    ...
    private function generateIdCard($employeeName, $nrp, $jabatan, $company, $gol_darah, $masa_berlaku, $personImagePath, $qr_code)
{
    ...
    // Ambil gambar border
    $addedImage = imagecreatefrompng("simper/border-qr.png");
    if (!$addedImage) {
        die('Failed to create image from the added image.');
    }
    ...
    // Tentukan path untuk menyimpan gambar
    $outputPath = 'uploads/simper/' . uniqid() . '.png';
...
public function postDelete()
    {
        ...
        if ($id) {
            $simperModel = new SimperModel();
            $unitRecomendationModel = new UnitRecomendationModel();

            // Ambil account_id dari admin yang akan dihapus
            $simper = $simperModel->find($id);
            if ($simper) {

                // Hapus data dari tabel admin
                $simperModel->delete($id);

                // Hapus data dari tabel accounts
                $unitRecomendationModel->where("simper_id", $id)->delete();

                return redirect()->to('/dept_head/simper')->with('success', 'Success! Simper berhasil dihapus.');
            } else {
                return redirect()->to('/dept_head/simper')->with('error', 'Failed! Simper tidak ditemukan.');
            }
        } else {
            return redirect()->to('/dept_head/simper')->with('error', 'Failed! ID tidak valid.');
        }
    }
...
//     $addedImage = imagecreatefromjpeg("simper/person.jpg");
//     if (!$addedImage) {
//         die('Failed to create image from the added image.');
//     }
