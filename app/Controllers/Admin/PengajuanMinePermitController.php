...
class PengajuanMinePermitController extends BaseController
{
    ...
    public function index(): string
    {
        ...
        $data = [
            ...
            'mine_permits_waiting' => $this->minePermitModel->where("approval_status", "waiting")->orderBy("created_at", "DESC")->findAll(),
            'mine_permits_accept' => $this->minePermitModel->where("approval_status", "approved")->findAll()
        ];
    
        // DD($data);
        // $data['company'] = $companyModel->find($companyId);
        // $data['anggota'] = $anggotaModel->getAnggotaData($companyId);
        return view('dept_head/pengajuan_id_card', $data);
    }
    ...
    public function detailPengajuan($id)
    {
        ...
        if($minePermit){
            ...
            return view('dept_head/detail-pengajuan.php', $data);
        }else{
    ...
    public function postContinueDone()
    {
        ...
        return redirect()->to("/dept_head/id_card/detail/id_card/". $id)->with("success", "Success! Mine permit telah selesai dibuat.");
    ...
    public function detail($id)
    {
        ...
        if($minePermit){
            ...
            return view('dept_head/detail.php', $data);
        }else{
            echo "id card tidak ditemukan";
        }
    }
    ...
    public function detailIDCard($id)
    {
        ...
        if($minePermit){
            ...
            return view('dept_head/detailIDCard.php', $data);
        }else{
            echo "id card tidak ditemukan";
        }
    }
    ...
    public function postInstanCreate()
    {
        ...
        return redirect()->to("/dept_head/id_card")->with("success", "Success! Mine permit berhasil ditambah.");
        // return redirect()->to("/" . $outputPathMinePermit)->with("success", "Success! mine permit berhasil ditambah.");
    }
    ...
    public function postAjukanMinePermit()
    {
        ...
        // Membuat QR code unik
        $qr_code = uniqid();
        // dd($imageName);
        
        // Generate path untuk mine permit
        $outputPathMinePermit = $this->generateIdCard(
            strtoupper($employeeName),
            strtoupper($nrp),
            strtoupper($jabatan),
            strtoupper($company),
            strtoupper($gol_darah),
            $masa_berlakuformat,
            $imageName,
            $qr_code
        );
        
        // Generate path untuk ID card belakang
        $outputPathMinePermitBelakang = $this->generateIdCardBelakang();
        ...
        $data = [
            ...
            'mine_permit' => $outputPathMinePermit, // Menyimpan path file
            'mine_permit_belakang' => "/" . $outputPathMinePermitBelakang, // Menyimpan path file
            'qr_code' => $qr_code, // Menyimpan QR code
            'approval_status' => "waiting", // Status approval
            'approval_soh' => "waiting", // Status approval
            'approval_esr' => "waiting", // Status approval
        ];
    ...
    {
        $backgroundPath = 'id_card/belakang_bg_compress.png';
...
public function postDelete()
    {
        ...
        if ($id) {
            ...
            if ($simper) {
                ...
                return redirect()->to('/dept_head/id_card')->with('success', 'Success! Mine Permit berhasil dihapus.');
                
            } else {
                return redirect()->to('/dept_head/id_card')->with('error', 'Failed! Mine Permit tidak ditemukan.');
            }
        } else {
            return redirect()->to('/dept_head/id_card')->with('error', 'Failed! ID tidak valid.');
        }
    }
}